...
from .converters.pptx_converter import PptxConverter, SlideResult
from .converters.xlsx_converter import XlsxConverter, SheetResult
...
class Router:
    ...
    # Magic Bytes（文件头签名）
    MAGIC_BYTES = {
        b'\xD0\xCF\x11\xE0': 'office',  # Office Open XML (docx/pptx/xlsx)
        b'%PDF': 'pdf',                  # PDF
    }
    ...
    def get_converter(self, format_type: str):
        ...
        Args:
            format_type: 'docx' | 'pdf' | 'pptx' | 'xlsx'
...
class ConversionPipeline:
    ...
    def __init__(self, db_path: str = "./wiki_db", output_dir: Optional[str] = None,
        ...
        # 初始化 file_registry（增量检测用）
        from .file_registry import FileRegistry
        self.registry = FileRegistry(db_path=str(self.db_path / "docstore.db"))
    ...
    def convert(self, filepath: str) -> ConversionResult:
        ...
        Args:
            filepath: 原始文件路径（.docx | .pdf | .pptx | .xlsx）
        ...
        # ==================== Step 0: 增量检测 ====================
        new_hash = self._compute_file_hash(str(filepath))
        old_record = self.registry.get_by_path(str(filepath))
        
        if old_record and old_record['content_hash'] == new_hash:
            print(f"⏭️ {filepath.name} 未变化，跳过转换")
            # 返回现有结果（从 MD 文件反推）
            return self._get_cached_result(str(filepath))
        
        # 如果文件变了，清理旧 MD 集合
        if old_record:
            self._cleanup_old_md(str(filepath))
            print(f"🧹 已清理旧 MD 集合")
        ...
        self.registry.mark_indexed(str(filepath), len(md_files))
    ...
    def _normalize_sections(self, converter_result: dict, format_type: str) -> List[dict]:
        ...
        if format_type == 'docx':
            ...
            for slide in converter_result.get('sections', []):
                sections.append({
                    'title': slide.title,
                    'content': slide.content,
                    'heading_level': 1,  # PPTX 扁平结构，统一为 L1
                    'parent_title': None,
                    'page_range': str(slide.slide_number),
                    'conversion_method': 'pptx',
                    'metadata': {**slide.metadata, 'has_notes': bool(slide.notes)},
                })
        
        elif format_type == 'xlsx':
            for sheet in converter_result.get('sections', []):
                sections.append({
                    'title': sheet.title,
                    'content': sheet.content,
                    'heading_level': 1,  # XLSX 扁平结构，统一为 L1
                    'parent_title': None,
                    'page_range': str(sheet.sheet_number),
                    'conversion_method': 'xlsx',
                    'metadata': {**sheet.metadata, 'rows': sheet.rows_count, 'cols': sheet.cols_count},
                })
    ...
    def _cleanup_old_md(self, filepath: str):
        """
        清理旧 MD 集合（避免残留文件）。
        ...
        doc_dir = Path(self.output_dir) / doc_name
        
        if doc_dir.exists():
            # 删除整个文档目录
            import shutil
            shutil.rmtree(doc_dir)
            print(f"   🗑️ 清理旧 MD: {doc_dir}")
    
    def _get_cached_result(self, filepath: str) -> ConversionResult:
        """
        从现有 MD 文件反推 ConversionResult（用于跳过转换时返回）。
        ...
        md_files = []
        if doc_dir.exists():
            md_files = [str(f) for f in doc_dir.rglob("*.md") if not f.name.startswith('.')]
        
        # 从 _index.md 提取基本信息（简化版）
        ...
        if index_file.exists():
            with open(index_file, 'r', encoding='utf-8') as f:
                ...
                # 构建虚拟 section（实际内容从 MD 文件读取）
                for md_file in md_files:
                    if "_index.md" not in md_file:
                        with open(md_file, 'r', encoding='utf-8') as mf:
                            ...
                            sections.append({
                                'title': title,
                                'content': md_content[200:],  # 跳过 frontmatter
                                'heading_level': 1,
                                'parent_title': None,
                                'page_range': '?',
                                'conversion_method': 'cached',
