            ...
            for file_path, content_hash in scan_result.items():
                ...
                conn.execute("""
                    ...
                    ON CONFLICT(file_path) DO UPDATE SET
                        content_hash = excluded.content_hash,
                        status = CASE
                            WHEN file_registry.content_hash != excluded.content_hash THEN 'pending'
                            ELSE file_registry.status
                        END,
                        indexed_at = CASE
                            WHEN file_registry.content_hash != excluded.content_hash THEN NULL
                            ELSE file_registry.indexed_at
                        END,
                        error_message = NULL
